...
from asyncbb.ethereum.contract import Contract
from asyncbb.ethereum.client import JsonRPCClient

from asyncbb.ethereum.test.parity import requires_parity
from asyncbb.ethereum.test.faucet import FaucetMixin, FAUCET_PRIVATE_KEY
...
class ContractTest(FaucetMixin, AsyncHandlerTest):
    ...
    async def test_deploy_contract(self, *, node):
        ...
        self.assertEqual(result[0], constructor_data[0])
    ...
    async def test_complex_contract(self, *, node):
        ...
        winner = winner[0][:winner[0].index(0)]

        self.assertEqual(winner, proposals[2])
